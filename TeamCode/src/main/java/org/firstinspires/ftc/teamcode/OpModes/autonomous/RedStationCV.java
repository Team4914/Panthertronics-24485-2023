...
@Autonomous
public class RedStationCV extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        Pose2d startPose = new Pose2d(-48 -1 + ROBOT_WIDTH/2,-72 + ROBOT_LENGTH/2, Math.PI/2);
        ...
        TrajectorySequence leftTapeTraj = drive.trajectorySequenceBuilder(startPose)
                .splineToLinearHeading(new Pose2d(-48, -24 - 9.5 - ARM_GROUND_LENGTH + CLAW_LENGTH/2 - ROBOT_LENGTH/2, Math.PI/2), Math.PI/2)
        ...
        TrajectorySequence middleTapeTraj = drive.trajectorySequenceBuilder(startPose)
                .splineToLinearHeading(new Pose2d(-48 + 14, -24 -0.5 - ARM_GROUND_LENGTH + CLAW_LENGTH/2 - ROBOT_LENGTH/2, Math.PI/2), Math.PI/2)
                .addTemporalMarker(arm::openClawLeft)
                .waitSeconds(1)
                .addTemporalMarker(() -> arm.setState(Arm2.State.BOARD))
                //.waitSeconds(1)
        ...
        TrajectorySequence rightTapeTraj = drive.trajectorySequenceBuilder(startPose)
                .splineToLinearHeading(
                        new Pose2d(
                                -24 - ROBOT_LENGTH/2 - ARM_GROUND_LENGTH - CLAW_LENGTH/2,
                                -24  - ROBOT_LENGTH/2 + 1,
                                0),
                        0
                )
